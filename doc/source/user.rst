...
particular process crosses a particualr threshold.
Say for example, you would like to set a 'warning'
alarm at 70% of CPU usage and a 'criticial' alarm at 90% CPU usage for the
Elasticsearh process.
...
You would just need to provide these three required arguments for defining new service checks:

  * A valid InfluxDB query that should return only one row with a single value.
    Check the `InfluxDB documentation <https://influxdb.com/docs/v0.9/query_language/index.html>`_
...
.. note:: Prior to enabling STARTTLS, you need to configured the *SMTP Authentication method*

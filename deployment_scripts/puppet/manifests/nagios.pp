...
if ! empty($influxdb_nodes){
  $influxdb_nodes_params = get_check_http_params($influxdb_nodes, 'influxdb_vip')

  $grafana_defaults = {
    port                       => hiera('lma::infrastructure_alerting::grafana_port'),
    url                        => '/login',
    custom_address             => 'internal_address',
    string_expected_in_content => 'grafana',
    service_description        => 'Grafana',
    require                    => Class['lma_infra_alerting::nagios::hosts'],
  }
  create_resources(lma_infra_alerting::nagios::check_http, $influxdb_nodes_params, $grafana_defaults)
...
if ! empty($es_kibana_nodes){
  $es_kibana_nodes_params = get_check_http_params($es_kibana_nodes, 'elasticsearch')
  $kibana_defaults = {
  ...
  create_resources(lma_infra_alerting::nagios::check_http, $es_kibana_nodes_params, $kibana_defaults)
  ...
  create_resources(lma_infra_alerting::nagios::check_http, $es_kibana_nodes_params, $es_defaults)
}

...
define lma_infra_alerting::nagios::check_http(
  ...
  $command = {
    "${check_command}" => {
      properties => {
        command_line => "${nagios::params::nagios_plugin_dir}/check_http -4 -I '${hostaddress}' ${base_options} ${port_option} ${url_option} ${expect_in_status_option} ${expect_in_content_option}",
      }
    }
  }

...
class lma_infra_alerting::nagios::check_ssh(
  ...
  $commands = {
    check_ssh_public => {
      properties => {
        command_line => "${nagios::params::nagios_pluigin_dir}/check_ssh '\$_HOSTPUBLIC_ADDRESS\$'",
      }
    },
    check_ssh_storage => {
      properties => {
        command_line => "${nagios::params::nagios_pluigin_dir}/check_ssh '\$_HOSTSTORAGE_ADDRESS\$'",
      }
    },
    check_ssh_private => {
      properties => {
        command_line => "${nagios::params::nagios_pluigin_dir}/check_ssh '\$_HOSTPRIVATE_ADDRESS\$'",
      }
    },
  }
  ...
  $service_checks = {
    ...
    'SSH public' => {
      properties => {
        hostgroup_name => $public_hostgroups,
        check_command => 'check_ssh_public',
      }
    },
    'SSH storage' => {
      properties => {

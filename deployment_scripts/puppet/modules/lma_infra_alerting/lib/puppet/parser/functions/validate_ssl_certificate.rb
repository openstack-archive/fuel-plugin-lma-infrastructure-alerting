...
module Puppet::Parser::Functions
    newfunction(:validate_ssl_certificate, :type => :rvalue) do |args|
        ...
        err_msg = `openssl x509 -noout -in #{args[0]}`
        raise "'#{args[0]}' is not a valid certificate" unless err_msg.chomp() == ""

        dates   = `openssl x509 -dates -noout -in #{args[0]}`.gsub("\n", '')
        subject = `openssl x509 -subject -noout -in #{args[0]}`.gsub("\n", '')
        pk      = `openssl rsa -check -noout -in #{args[0]}`.gsub("\n",'')

        cn        = subject.match(/CN=(#{args[1]})\/?.*$/)
        search_cn = subject.match(/CN=(.*)/)[0]
        ...
        raise "Not Found 'CN=#{args[1]}' within #{search_cn}" unless cn

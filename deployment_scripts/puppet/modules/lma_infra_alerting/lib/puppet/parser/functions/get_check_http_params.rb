...
- Array of nodes, each node being described as a hash.
- Key identifying the node's name.
- The network role mapping to the checked IP address.

Example:

  $hash = :get_check_http_params(
    [{'name' => 'node-1', 'network_roles' => { 'influxdb_vip' => '10.109.2.3'},
     {'name' => 'node-2', 'network_roles' => { 'influxdb_vip' => '10.109.2.4'}
    'name', 'influxdb_vip'
  )

Would return:

  [{'host_name' => 'node-1', 'custom_address' => '10.109.2.3'},
   {'host_name' => 'node-2', 'custom_address' => '10.109.2.4'}]
    ...
    nodes.keys.sort.each do |k|
        ...
        result << {'host_name' => nodes[k][node_name], 'custom_address' => ip}
    end
